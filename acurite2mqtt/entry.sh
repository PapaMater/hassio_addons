...
echo "SDR_DEVICE_INDEX =" $SDR_DEVICE_INDEX
echo "DEBUG =" $DEBUG



rtl_433  $PROTOCOL -d $SDR_DEVICE_INDEX -C $UNITS  -F mqtt://$MQTT_HOST:$MQTT_PORT,user=$MQTT_USERNAME,pass=$MQTT_PASSWORD,retain=$MQTT_RETAIN,events=$MQTT_TOPIC/events,states=$MQTT_TOPIC/states,devices=$MQTT_TOPIC[/model][/id][/channel:0]  -M time:tz:local -M protocol -M level | /scripts/rtl_433_mqtt_hass.py
